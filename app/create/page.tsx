  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        setMsg({ isOpen: true, title: "延續時光", content: "已為您載入往日設定，選個新日期即可再次啟程。", type: "success" });
      ...
      <main className="max-w-xl mx-auto p-6 mt-8">
        <form onSubmit={handleCreate} className="bg-white border border-stone p-8 space-y-6 shadow-sm text-ink font-sans">
          ...
          <div>
            ...
            <input required type="text" list="location-options" value={newSession.location} onChange={(e) => setNewSession({ ...newSession, location: e.target.value })} className="w-full bg-sage/5 border border-sage/10 p-2 focus:outline-none rounded-sm transition-all" placeholder="手動輸入球館名稱或選擇下方建議" />
